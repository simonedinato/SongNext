<?php
	...
	/**
	 * Check if the session variable 'nomeu' has a value
...
?>
		...
		<div id="canzoni" class="menu-c">
			...
			<?php
                /**
                 * Select all the songs from the database where the song's owner is the current user
                 */
                $sql="select * from canzone where nome_u='".$_SESSION['nomeu']."' ;";
                $result=mysqli_query($conn,$sql);
                if(!$result){
                    /**
                     * If the query fails, display an error message with the error number and message
                     */
                ...
                while($row=mysqli_fetch_assoc($result)){
                    /** 
                     * Select all likes for the current song
                     */
                    $sql="select * from slike where codc=".$row["codc"].";";
                    /**
                     * Select all likes for the current song made by the current user
                     */
                    $sql1="select * from slike where codc=".$row["codc"]." and nome_u='".$_SESSION['nomeu']."';";
                    /**
                     * Execute both of the above SQL statements
                     */
                    $result1=mysqli_query($conn, $sql);
                    $result2=mysqli_query($conn, $sql1);
                    /**
                     * Check if there are any rows in the result of the second SQL statement (i.e. if the user has already liked this song)
                     */
                    if(mysqli_num_rows($result2)>0){
                        /**
                         * If the user has liked the song, set the fill color to red
                         */
                        $fill="#e56b6b";
                    }
                    else {
                        /**
                         * If the user has not liked the song, set the fill color to green
                         */
                        $fill="#a8b773";
                    }
